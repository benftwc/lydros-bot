...
async def handle_command(command, args, message, bot_client):
    ...
    # print(f"{message.author.name}: run {command.upper()} command with\n{args}")

    # Retrieve the command
    cmd_obj = COMMAND_HANDLERS[command]
    true_args_count = len([p for p in cmd_obj.params if isinstance(p, str)])

    if cmd_obj.params and len(args) < true_args_count:
